...
func (c *Collection) exec() error {
	...
	// 执行sql
	err = run(tx, c.Opinions.BeforeCreateSql...)
	if err != nil {
		log.Println(1)
		return err
	}
	...
	err = run(tx, c.Opinions.AfterCreateSql...)
	if err != nil {
		return err
	}
	err = run(tx, c.Opinions.BeforeInsertSql...)
	if err != nil {
		return err
	}
	...
	err = run(tx, c.Opinions.AfterInsertSql...)
	if err != nil {
		return err
	}
	tx.Commit()
...
func (c *Collection) query(query string) error {
	...
	c.Content, err = transContent(rows)
	return err
}

// 根据 query 返回的 rows 制造 Content
func transContent(rows *sql.Rows) (*Content, error) {
	content := &Content{}
	var err error
	defer rows.Close()
	content.Columns, err = rows.ColumnTypes()
	column, _ := rows.Columns()
	length := len(content.Columns)
	for rows.Next() {
		...
		for i := 0; i < length; i++ {
			if bytes, ok := values[i].([]byte); ok {
				mp[column[i]] = string(bytes)
				continue
			}
			mp[column[i]] = values[i]
		}
		content.Values = append(content.Values, mp)
	}
	return content, err
}
